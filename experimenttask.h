...
#define DAQmxErrChk( functionName, functionCall ) { int error; if ( DAQmxFailed( error = (functionCall) ) ) { qDebug() << "DAQmxErrChk" << functionName << "errorCode =" << error; throw ExperimentException( error ); } }
